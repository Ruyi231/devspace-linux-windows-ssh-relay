...
install_basic_packages() {
  echo "[1/4] Checking basic packages..."
  local missing=0
...
install_node() {
  echo "[2/4] Checking local Node.js..."
...
install_devspace() {
  echo "[3/4] Installing DevSpace CLI..."
...
initialize_config() {
  echo "[4/4] Creating config.env..."
  ensure_config
  echo "      Config file: ${CONFIG_FILE}"
  echo
  echo "Install complete. Next steps:"
  echo "  1) Set project root: ./set_project.sh /path/to/your/project"
  echo "  2) Save the Windows ngrok URL: ./set_relay_public_url.sh https://example.ngrok-free.dev"
  echo "  3) Start service: ./start_relay_target.sh"
  echo
